...
class WebFormAdvanced(WebsiteGenerator):
	website = frappe._dict(
		template = "templates/generators/web_form_advanced.html",
...
""")
	...
	def get_context(self, context):
		...
		if frappe.form_dict.name:
			...
			context.title = context.title + ": " + context.doc.get(context.doc.meta.get_title_field())
			context.doc.add_seen()

			context.reference_doctype = context.doc.doctype
			context.reference_name = context.doc.name

		context.child_docfields = dict()
		context.child_doclayouts = dict()
				
		for df in frappe.get_meta(self.doc_type).get_table_fields():
			#for do in frappe.get_meta(df.options).fields:
			fields = frappe.get_meta(df.options).fields
			if fields and df.options:
				context.child_docfields[df.options] = frappe.get_meta(df.options).fields
				context.child_doclayouts[df.options] = self.get_layout_child(df.options)
	...
	def get_layout_child(self, childTable):
		layout = []
		for df in frappe.get_meta(childTable).fields:
			if df.fieldtype=="Section Break" or not layout:
				layout.append([])
...
def accept():
	...
	web_form = frappe.get_doc("Web Form Advanced", args.web_form)
...
def delete(web_form, name):
	web_form = frappe.get_doc("Web Form Advanced", web_form)
