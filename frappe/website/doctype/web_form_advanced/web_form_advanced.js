	...
	set_fieldname_select: function(frm) {
		...
		if(doc.doc_type) {
			frappe.model.with_doctype(doc.doc_type, function() {
				...
				frappe.meta.get_docfield("Web Form Field Advanced", "fieldname", frm.doc.name).options
					= [""].concat(fields);
			});
		}
	}
...
frappe.ui.form.on("Web Form Advanced", {
	refresh: function(frm) {
		// show is-standard only if developer mode
		frm.get_field("is_standard").toggle(frappe.boot.developer_mode);

		frappe.web_form_advanced.set_fieldname_select(frm);
...
frappe.ui.form.on("Web Form Field Advanced", "fieldname", function(frm, doctype, name) {
	...
	doc.fieldtype = frappe.meta.get_docfield("Web Form Field Advanced", "fieldtype")
